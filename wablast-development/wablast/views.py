...
def send_message(data):
    ...
    time.sleep(3)    # Should have used WebDriverWait or redundant and use after try instead
    ...
    except Exception:  # Should be moved to bottom as it covers most of the script
    ...
    time.sleep(5)   # WebDriverWait(browser, 5).until(EC.element_to_be_clickable((By.CSS_SELECTOR, Selectors.MESSAGE_BOX)))
...
def send(request):
    if request.method == 'GET': 
        print(request.GET)  # Should use logger
        data = request.GET
        return send_message(data)

    if request.method == 'POST': # Include Try and Except if invalid JSON Data received
        data = request.body  # byte
        data = json.loads(data.decode('utf-8'))  # json/dict
        return send_message(data) # Use logger to received POST request

    else:
        return HttpResponse(status=405) # Use logger Method not allowed
    ...
    for index, row in random_df.iterrows():
        ...
        if not is_success:
            ...
            try:
                ...
                # Write code here to add time after soceng
